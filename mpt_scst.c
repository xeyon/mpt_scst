...
#define MPT_VERSION_STRING "Version 3.1.20(Special mod for LSI 53C1030)"
...
static int mpt_stm_adapter_offline(MPT_STM_PRIV *priv);
	...
	if (ioc->bus_type == SPI) {
		...
		for (i = 0; i < 16; i++) {
			seq_printf(seq, " DeviceSeting %02d: 0x%x 0x%x 0x%x\n",
				   priv->SCSIPortPage2.DeviceSettings[i].Timeout,sess
...
static ssize_t mpt_version_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf);


struct kobj_attribute mpt_version_attr =
	__ATTR(version, S_IRUGO, mpt_version_show, NULL);

static const struct attribute *mptt_attrs[] = {
	&mpt_version_attr.attr,
	NULL,
};

static ssize_t mpt_hw_target_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf);

static struct kobj_attribute mpt_hw_target_attr =
	__ATTR(hw_target, S_IRUGO, mpt_hw_target_show, NULL);

static ssize_t mpt_port_id_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf);

static ssize_t mpt_port_id_store(struct kobject *kobj,
	struct kobj_attribute *attr, const char *buffer, size_t size);

static struct kobj_attribute mpt_port_id_attr =
	__ATTR(port_id, S_IRUGO|S_IWUSR, mpt_port_id_show,
		mpt_port_id_store);

static ssize_t mpt_capabilities_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf);

static struct kobj_attribute mpt_capabilities_attr =
	__ATTR(caps, S_IRUGO, mpt_capabilities_show, NULL);

static ssize_t mpt_physical_interface_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf);

static struct kobj_attribute mpt_physical_interface_attr =
	__ATTR(phy_intf, S_IRUGO, mpt_physical_interface_show, NULL);

static ssize_t mpt_sync_factor_show(struct kobject *kobj,
	struct kobj_attribute *attr, char *buf);

static struct kobj_attribute mpt_sync_factor_attr =
	__ATTR(sync, S_IRUGO, mpt_sync_factor_show, NULL);

static const struct attribute *mpt_hw_tgt_attrs[] = {
	&mpt_hw_target_attr.attr,
	&mpt_port_id_attr.attr,
	&mpt_capabilities_attr.attr,
	&mpt_physical_interface_attr.attr,
	&mpt_sync_factor_attr.attr,
	NULL,
};
...
static struct scst_tgt_template tgt_template = {
	...
	.get_initiator_port_transport_id = mpt_get_transport_id,
	.tgtt_attrs = mptt_attrs,
};
...
static int mptstm_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	/* Register scst target name as "iocN", this is to identify
	 * mutiply HBAs on the system */
	tgt->scst_tgt = scst_register_target(&tgt_template, ioc->name);
	...
	/* Add sysfs attributes */
	{
		int i = 0;
		while (1) {
			const struct attribute *a = mpt_hw_tgt_attrs[i];
			if (a == NULL)
				break;
			rc = sysfs_create_file(scst_sysfs_get_tgt_kobj(tgt->scst_tgt), a);
			if (rc != 0)
				PRINT_ERROR(MYNAM ": Unable to create "
					"\"%s\" file for target %s",
					a->name, scst_get_tgt_name(tgt->scst_tgt));
			i++;
		}
	}
#endif
...
static int mpt_enable_tgt(struct scst_tgt *scst_tgt, bool enable)
{
	...
	if (enable) {
		if (tgt->target_enable != 1) {
			mpt_stm_adapter_online(mpt_stm_priv[ioc->id]);
			tgt->target_enable = 1;
			//TRACE_DBG("Enabled Target, %d, %d", ioc->id, tgt->target_enable);
		}
	}
	else {
		if (tgt->target_enable != 0) {
			mpt_stm_adapter_offline(mpt_stm_priv[ioc->id]);
			tgt->target_enable = 0;
			//TRACE_DBG("Disabled Target %d, %d", ioc->id, tgt->target_enable);
		}
	}
	res = 0;
...
static bool mpt_is_tgt_enabled(struct scst_tgt *scst_tgt)
{
	...
	TRACE_DBG("%p target_enabled: %d", tgt, tgt->target_enable);
					    ...
					    struct mpt_cmd *mpt_cmd)
{
	...
	/* let work_id = initiator id to indicate the port is occupied */
	priv->work_id = init_index;
...
/*	TRACE_DBG("%s: CMD %p, re_word %x, alias %x, lun %x, tag %x,"
	...
	TRACE_DBG("scst_cmd[%d] %p, op %s",
			index, priv->scst_cmd[index],
				   ...
				   int result)
{
	...
	if (result == 0) {
		...
		__clear_bit(MPT_SESS_PENDING, &sess->sess_flags);
	} else {
...
/**
 * mpt_send_cmd_to_scst() -  send the received cmd to scst core
 * @cmd:	mpt_cmd struct that hold the received cmd
 * @context:	scst_exec_context
 *
 * Description:
 *    This calls _stm_target_command() to create a new SCST command,
 *    and calls scst_cmd_init_done() to send the cmd to scst core.
 */
static int mpt_send_cmd_to_scst(struct mpt_cmd *cmd,
				enum scst_exec_context context)
{
...
static void stm_tgt_reply(MPT_ADAPTER *ioc, u32 reply_word)
{
	...
	TRACE_DBG("io_state[%d] = %02x, init_idx %x, scst cmd %p",
			index, *io_state, init_index, scst_cmd);
	/*
	...
	if (*io_state & IO_STATE_STATUS_SENT) {
		...
		*io_state &= ~IO_STATE_STATUS_SENT;
		/*mpt_free_msg_frame(_HANDLE_IOC_ID, priv->current_mf[index]);
		 * This is the corrective of BUG5 by xusy@zhftc.com */
		mpt_msg_frame_free(priv, index);
		if (*io_state & IO_STATE_DATA_SENT) {
			*io_state &= ~IO_STATE_DATA_SENT;
			stm_data_done(ioc, reply_word, scst_cmd, cmd, index);
		...
		if ((*io_state & ~IO_STATE_HIGH_PRIORITY) == IO_STATE_AUTO_REPOST) {
			TRACE_DBG("%s", "IO_STATE_AUTO_REPOST");
			*io_state = IO_STATE_POSTED;
		} else if ((*io_state & ~IO_STATE_HIGH_PRIORITY) == 0) {
		...
		/* End of cmd life */
		/* work_id = port_id -- this port is ready for next cmd transfer*/
		priv->work_id = priv->port_id;
		/* TODO: for now, I just consider SCSI addr ranged from 7 to 0,
		 * and of cause this is only applicable in an SPI system */
		for(i = 7; i >= 0; i--) {
			struct mpt_sess *sess = priv->tgt->sess[i];
			if((sess != NULL) &&
					test_bit(MPT_SESS_PENDING, &sess->sess_flags)) {
				TRACE_DBG("Restart pended session[%d]", i);
				mpt_alloc_session_done(sess->scst_sess, sess, 0);
			}
		}
		/*
	...
	/*
	 * in multiple initiator system, an Ext Bus Reset from other initiator may interrupt
	 * current transaction and cause the scst_cmd can't normally release. Here is to reset
	 * the command state and assume the conversation finished and free up.
	 */
	if (*io_state & IO_STATE_POSTED) {
		TRACE_DBG("WARNING: IO_STATE_POSTED, release scst_cmd %p",
				scst_cmd);
		/*
		 *  command buffer was posted, so we now have a SCSI command
		 *  However, we just ignore the new command for now.
		 */
		priv->scst_cmd[index] = NULL;
	...
	TRACE_DBG("FAILURE! Exception handling mpt_cmd %p state = %02x",
			cmd, cmd->state);
	TRACE_DBG("io_state[%d] = %02x, initiator %x, scst cmd %p",
			index, *io_state, init_index, scst_cmd);
 ...
 * stmapp_tgt_command
 *
 * this function is intended to allocate mpt_cmd struct.
 * it also creates new session to correspond initiator,
 * if not yet existed. Then the cmd is sent to scst for
 * execution.
 */
static void stmapp_tgt_command(MPT_STM_PRIV *priv, u32 reply_word)
{
	...
	/* Allocate mem of mpt_cmd struct */
	...
	if (test_bit(MPT_SESS_INITING, &sess->sess_flags)) {
		list_add_tail(&cmd->delayed_cmds_entry, &sess->delayed_cmds);
	} else if ((priv->work_id != priv->port_id) &&
			(priv->work_id != init_index)) {
		/* work_id != port_id & init_index means the port is currently
		 * serving other initiator. Put the cmd in the session delayed
		 * queue now, and later when the port is idle, we'll call
		 * mpt_alloc_session_done() to launch it again */
		TRACE_DBG("session[%d] pending while initiator %d runing",
				init_index, priv->work_id);
		list_add_tail(&cmd->delayed_cmds_entry, &sess->delayed_cmds);
		__set_bit(MPT_SESS_PENDING, &sess->sess_flags);
	} else {
...
static int mpt_target_release(struct scst_tgt *scst_tgt)
{
	...
	TRACE_ENTRY();
	TRACE_DBG("On MPT target release...");

	if (mpt_is_tgt_enabled(scst_tgt)) {
		res = mpt_enable_tgt(scst_tgt, false);
	}
	...
	uint8_t **result)
{
	...
	struct {
		u8	format_proto;	/* format and protocol ID (1 for SPI) */
		u8	__resv1;
		u16	scsi_addr;		/* SCSI address */
		u16	__resv2;
		u16	rel_tgt_id;	/* relative target port ID */
		u8	__resv3[16];
	} __attribute__((__packed__)) *id;

	if (!scst_sess)
		return SCSI_TRANSPORTID_PROTOCOLID_SPI5;
	...
	id->format_proto = SCSI_TRANSPORTID_PROTOCOLID_SPI5;
	id->scsi_addr = 0x0001 << (sess->init_index);
	id->rel_tgt_id = tgt->rel_tgt_id;
	*result = (uint8_t *)id;
	return 0;
}
 ...
 * this is assured by setting tgt_template.sg_tablesize = NUM_SGES
 *
 */
...
/*out:*/
	TRACE_BUFFER("Sense Data", info, len);
			     ...
			     int length, int offset)
{
	...
	TRACE_DBG("Request %p Fn(%02x) sent, io_state[%d]=%4x",
...
static int mpt_xmit_response(struct scst_cmd *scst_cmd)
{
	...
	TRACE_DBG("scst_cmd_status=%02x, is_send_status=%x, %d, %d", prm.rq_result,
	...
	} /*else if (((prm.rq_result == 0x08) || (prm.rq_result == 0x28))
			&& (prm.bufflen == 0) && (prm.cmd->scst_cmd->cdb[7] > 7)) {
		TRACE_DBG("fake \"Logical Block Address Out Of Range\" sense data");
		 sometimes scst send BUSY/QUEUE FULL because of allocation of
		 * large memory blocks, this is to tell the initiator not to try
		 * that big amount of blocks any more
		stmapp_set_sense_info(prm.tgt->priv, prm.cmd->CMD,
				0x05, 0x21, 0x00);
	}*/
	...
	/* STATUS phase */
	{
		...
		if (prm.rq_result == 0) {
			...
			mpt_trace_flag |= TRACE_BUFF;
			TRACE_BUFFER("CDB:", scst_cmd->cdb_buf, scst_cmd->cdb_len);
			/* store status code in CMD->rsp->Status */
			if (IsScsi(priv)) {
				SCSI_RSP *rsp = (SCSI_RSP *)prm.cmd->CMD->rsp;
				rsp->Status = prm.rq_result;
			}
		}
		...
		TRACE_DBG("io_state[%d] %02x, scst_cmd %p, flags %02x",
				index, priv->io_state[index], scst_cmd, flags);
 ...
 * Turn on WBus16 and Sync bits for 16-bit sync transfer
 */
static void
mpt_inquiry_no_tagged_commands(MPT_STM_PRIV *priv, struct scst_cmd *scst_cmd)
{
	...
	if (IsScsi(priv) && (scst_cmd->cdb[0] == INQUIRY) &&
		...
		if (length >= 8) {
/*			TRACE_DBG("clearing BQUE + CMDQUE and setting WBus16 + Sync 0x%p",
					address);*/
			address[6] &= ~0x80; /* turn off BQUE */
			address[6] |= 0x10;  /* turn on MultiP */
			address[7] &= ~0x02; /* turn off CMDQUE */
			address[7] |= 0x30;	 /* turn on WBus16 and Sync */
			TRACE_DBG("INQUIRY data Byte 6-7:0x%2x, 0x%2x",
								address[6], address[7]);
		}
				...
				int task_mgmt, int _lun)
{
	...
	if (test_bit(MPT_SESS_INITING, &sess->sess_flags)) {
		...
		mpt_local_task_mgmt(sess, task_mgmt, _lun);
		res = -EFAULT;
		goto out;
	}
		     ...
		     MPT_FRAME_HDR *mf_rep)
{
	...
	TRACE_DBG("%s: mf_req_idx=%d, Fn(%02x), ioc_status %04x",
		  ioc->name, MF_TO_INDEX(mf_req), rep->Function, ioc_status);
...
static void stm_cmd_buf_post(MPT_STM_PRIV *priv, int index)
{
	...
	spin_unlock_irq(&priv->hw_lock);
/*
	TRACE_DBG("buffer post req sent in req %p Fn(%02x), io_state[%d]=%04x",
			req, req->Function, index, priv->io_state[index]);
*/
...
static int mpt_stm_adapter_offline(MPT_STM_PRIV *priv)
{
	...
	for(i = 0; i < 256; i++) {
		if(priv->tgt->sess[i] != NULL) {
			sess = priv->tgt->sess[i];
			TRACE_DBG("unregistering scst_session %p", sess->scst_sess);
			scst_unregister_session(sess->scst_sess, true, NULL);
			TRACE_DBG("flushing delayed_cmds in mpt_session %p",
					sess);
			mpt_local_task_mgmt(sess, IMM_NTFY_TARGET_RESET, 0);
			priv->tgt->sess[i] = NULL;
			kfree(sess);
		}
	}
	atomic_set(&priv->tgt->sess_count, 0);

	/* abort adapter target mode */
	if (mpt_GetIocState(ioc, 1) == MPI_IOC_STATE_OPERATIONAL) {
...
#ifdef DEBUG_WORK_IN_THREAD
	strcat(buf, "DEBUG_WORK_IN_THREAD\n");
#endif
	...
	struct kobj_attribute *attr, char *buffer)
{
	...
	/* mark as key attribute when port_id is not 1 */
	if (priv->port_id != 1)
	...
	struct kobj_attribute *attr, const char *buffer, size_t size)
{
	...
	struct mpt_tgt *tgt;
	MPT_STM_PRIV *priv;
	int res = -E_TGT_PRIV_NOT_YET_SET, old, new;
	...
	old = priv->port_id;
	if (new < MPT_MAX_SCSI_DEVICES) {
		if (IsScsi(tgt->priv)) {
			/*TRACE_DBG("Changing target id from %d to %d\n",
					old, new); */
			priv->port_id = new;
			priv->work_id = priv->port_id;
			stm_set_scsi_port_page1(priv, NO_SLEEP);
			res = size;
		}
	}
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	cap = priv->SCSIPortPage0.Capabilities;
	sprintf(buf, "0x%08x\n", cap);
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	intf = priv->SCSIPortPage0.PhysicalInterface;
	sprintf(buf, "0x%08x\n", intf);
	...
	struct kobj_attribute *attr, char *buf)
{
	...
	/* Read in SCSIPortPage2 config*/
	memset(priv->hw->config_buf, 0, sizeof(priv->hw->config_buf));
	if (stm_get_config_page(priv, MPI_CONFIG_PAGETYPE_SCSI_PORT, 2, 0, NO_SLEEP))
		return -1;
	memcpy(&priv->SCSIPortPage2, priv->hw->config_buf, sizeof(SCSIPortPage2_t));

	for (i = 0; i < 16; i++) {
		sprintf(seq, " DeviceSeting %02d: 0x%02x 0x%02x 0x%04x\n", i,
...
#endif /* Routines to support Sysfs */
...
{
	TRACE_ENTRY();
	TRACE_DBG("On mpt_scst module exit...");
...
MODULE_DESCRIPTION("Fusion MPT SCSI Target Mode Driver for SCST");
MODULE_LICENSE("GPL");
MODULE_VERSION(MPT_VERSION_STRING);
